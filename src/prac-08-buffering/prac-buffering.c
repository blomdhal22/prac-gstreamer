 #define LOG_TAG      "prac-08-buffering"
 #define LOG_NDEBUG   0
 #include <logging.h>
...
buffer_timeout (gpointer data)
{
  ...
  DD("is_buffering=%s, target_state=%d", is_buffering ? "true" : "false", target_state);
...
on_message_buffering (GstBus *bus, GstMessage *message, gpointer user_data)
{
  ...
  DD("%s, percent=%d", GST_MESSAGE_SRC_NAME(message), percent);
...
// ex) ../out/bin/prac-08-buffering file:/home/daniel/Videos/audio.ogg
// ex) ../out/bin/prac-08-buffering https://gstreamer.freedesktop.org/data/media/sintel_trailer-480p.webm
gint
main (gint   argc,
      gchar *argv[])
{
